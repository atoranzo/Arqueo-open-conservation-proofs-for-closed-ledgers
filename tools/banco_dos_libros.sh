...
for PAR in "A:$V_A" "B:$V_B"; do
  L="${PAR%%:*}"; V="${PAR#*:}"
  # RFC-0007 E1b (§452): el nodo emite v5; el sobre de conflicto acepta v4, v5 o v6 (el mando lo deriva).
  case "$(qnum "$(campo "$V" result.formatVersion)")" in 4|5|6) : ;;
    *) fallo "la cabeza del libro $L no es v4, v5 ni v6: sin consRoot no hay nada que comparar";; esac
done
...
for PAR in "A:$N_A" "B:$N_B"; do
  L="${PAR%%:*}"; V="${PAR#*:}"
  case "$(qnum "$(campo "$V" result.formatVersion)")" in 4|5|6) : ;; *) fallo "la cabeza nueva de $L no es v4, v5 ni v6";; esac
