...
// SPIKE-B-ETAPA2A. El cociente oculto (nota 2024/1037, 4.2): con m > 0 los trozos avanzan de
...
fn segmentar<E: FieldElement>(
    ...
    let s = trace_len - m;
    assert!(
        coefficients.iter().skip(s * num_cols).all(|c| *c == E::ZERO),
        "cociente oculto: el polinomio no cabe en {} trozos de paso {}",
        num_cols,
        s
    );
    ...
    for i in 0..num_cols {
        let mut t: Vec<E> = (0..trace_len).map(|_| E::ZERO).collect();
        for k in 0..s {
            if let Some(&c) = coefficients.get(i * s + k) {
                t[k] = c;
            }
        }
        for k in 0..m {
        ...
        if i + 1 < num_cols {
            let r: Vec<E> = (0..m)
                .map(|_| match moneda.draw::<E>() {
                    Ok(v) => v,
                    Err(_) => panic!("cociente oculto: la moneda no dio un elemento"),
                })
                .collect();
            for k in 0..m {
                t[s + k] = t[s + k] + r[k];
