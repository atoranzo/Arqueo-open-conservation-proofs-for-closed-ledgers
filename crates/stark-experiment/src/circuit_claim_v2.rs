//! **Reclamacion v2** (RFC-0003, S347) - EN PARALELO a `circuit_claim`,
//! que no se toca: la convivencia manda que el 0.2 siga IDENTICO byte a
//! byte, y el legado es inmune por dominio (RFC-0003), asi que ambos
//! circuitos viven mientras existan pendientes de su forma.
//!
//! Lo UNICO que cambia respecto del v1 es la forma del pendiente: aqui
...
//! el receptor lo recibe OPACO en el aviso y no necesita abrirlo -
//! `f` y `delta` jamas salen del testigo del PAGADOR (eso es materia
//! de `circuit_refund_v2`). La cadena del pendiente gana un CUARTO
//! merge:
//!
...
//!
//! El sobre entra por `COL_X` (55..59) y se LEE en su unico enlace -
//! el patron del salt de hoja (S117): un testigo de un solo enlace no
//! se transporta; un X' distinto desvia C2 y la raiz declarada lo
//! rechaza. La asimetria con `circuit_refund_v2` (alli X se produce en
...
//! de layout barre por fichero: dos Air en uno mezclarian sus ranuras.
...
use crate::circuit_claim::ClaimPublicInputs;
...
/// Identidad publica del receptor (muerta tras S39.1; se conserva el
/// calco del v1: el compromiso usa COL_ACC_ID, atada por C_PK_CHECK).
...
const COL_X: usize = 55; // 55..59
pub const TRACE_WIDTH: usize = 61;
...
pub const COL_FACC: usize = 60;

// ===== Filas =====
//
// Geometria derivada (playbook R2): cada tramo arranca en un ciclo
...
/// Raiz de pendientes. Filas 568..823 en la subida.
...
const C_ENV_DIG: usize = C_SALT_IN_B + 4; // 4
...
const C_IACC_STEP: usize = C_ENV_IN + 4; // 1
...
/// Fila de enlace del cuarto merge.
...
/// Construye la traza de una reclamacion v2.
///
/// Identica a la del v1 salvo el parametro `sobre` (el `X` opaco del
/// aviso) y el cuarto merge que lo absorbe.
...
pub fn build_trace(
    ...
    receiver_id: Digest,
    salt: Digest,
    // **El SOBRE `X`** (testigo): llega opaco en el aviso; el receptor
    // no lo abre. `C2 = M(C1, X)`.
    sobre: Digest,
    ...
    // EL SALDO SUBE: el dinero vuelve del pendiente a la cuenta.
    ...
    for r in 0..ROW_PENDING_ROOT {
        ...
        if pos < NUM_ROUNDS {
            ...
            match r {
                ROW_LEAF_LINK => {
                    // El nonce NO cambia: cobrar no consume el derecho.
                ...
                ROW_SALT_LINK => {
                    // EL TERCER MERGE de la hoja (S117): digest
                    // arrastrado, el rate := los CUATRO limbos del salt.
                    state_a[4..8].copy_from_slice(&digest_a);
                ...
                ROW_FROZEN_ROOT => {
                    // COMPROMISO INTERNO: H(identidad, aleatorio). La
                    // identidad es la de la cuenta que cobra (S39).
                ...
                ROW_PEND_ENV => {
                    ...
                    // arrastrado y el rate := el sobre, leido de COL_X -
                    // el patron del salt de hoja aplicado al pendiente.
                    state_a[4..8].copy_from_slice(&digest_a);
                ...
                ROW_PENDING_ENTRY => {
                    ...
                    //
                    // Carril A: C2 -> raiz ANTES (el pendiente existia).
                ...
                ROW_PK_DONE => {
                    // ENTRADA AL ARBOL DE CONGELADOS: hoja CERO.
                    let libre: Digest = [zero; 4];
...
pub struct ClaimAirV2 {
    context: AirContext<BaseElement>,
    pub_inputs: ClaimPublicInputs,
}

impl Air for ClaimAirV2 {
    type BaseField = BaseElement;
    // Las entradas publicas del v1 SOBREVIVEN: el sobre es testigo y
    // jamas se publica.
    ...
    fn new(trace_info: TraceInfo, pub_inputs: Self::PublicInputs, options: ProofOptions) -> Self {
        ...
        // Saldo (1), suministro (1), transporte (18), identidad
        // constante (4) = 24, grado 1 sin ciclo.
        for _ in 0..24 {
        ...
        // El cuarto merge (8): gate periodico x expresion lineal.
        for _ in 0..8 {
            degrees.push(TransitionConstraintDegree::with_cycles(1, full.clone()));
        }
    ...
    fn evaluate_transition<E: FieldElement<BaseField = Self::BaseField>>(
        ...
        // El tercer merge de la hoja (S117), cosido por `link_salt`.
        for i in 0..4 {
        ...
        // Los CUATRO elementos de la clave, en los dos carriles (S92.2).
        for i in 0..4 {
        ...
        // TITULARIDAD (S39): la identidad derivada es la de la cuenta.
        for i in 0..4 {
        ...
        // La identidad del receptor y el aleatorio tambien constantes
        // (entrada 36 / S50.7: la constancia del salt es solidez).
        for i in 0..4 {
        ...
        // ===== EL PENDIENTE (cuatro fases en v2) =====
        ...
        for i in 0..4 {
            ...
            // suma (sumar comprobaciones independientes las anula). La
            // identidad es la de la cuenta que cobra (S39).
            ...
            // C1: el digest interno, y el importe.
            result[C_PEND_VAL + i] = pend_val * (next[4 + i] - current[4 + i]);

            // EL CUARTO MERGE: C2 = M(C1, X). Digest arrastrado y el
            // rate := el sobre, leido de COL_X (carril A, precedente
            // C_PEND_VAL).
        ...
        for i in 0..4 {
            ...
            // EL PENDIENTE EXISTIA: el carril A entra con C2.
            result[C_PEND_ENTRY_A + i] = pend_entry
                * ((E::ONE - pbit) * (next[4 + i] - current[4 + i])
                    + pbit * (next[8 + i] - current[4 + i]));
            // Y el B con CERO: consumido, no se cobra dos veces.
    ...
    fn get_assertions(&self) -> Vec<Assertion<Self::BaseField>> {
        let zero = BaseElement::ZERO;
        let mut a = Vec::with_capacity(43);
        ...
        // Las raices del arbol de pendientes: antes con C2, despues sin.
        for i in 0..4 {
            a.push(Assertion::single(
                4 + i,
                ROW_PENDING_ROOT,
...
mod tests {
    ...
    use crate::native::{derive_leaf_salt_wide, derive_public_id_wide, native_climb,
        native_leaf_salted};
    ...
    /// C1 nativo del escenario: M(M(identidad, aleatorio), importe).
    fn c1_de(s: &Scenario) -> Digest {
        native_merge(
    ...
    fn climb_pending_de(s: &Scenario, hoja: Digest) -> Digest {
        let mut cur = hoja;
        for level in 0..TREE_DEPTH {
    ...
    fn scenario(balance: u64, amount: u64, supply_old: u64) -> Scenario {
        ...
        // La identidad del compromiso es la de la cuenta que cobra (S39).
        ...
        // El sobre X, opaco: al receptor le llega hecho en el aviso.
        let sobre: Digest = [
            BaseElement::new(0x50B2_0001),
            BaseElement::new(0x50B2_0002),
            BaseElement::new(0x50B2_0003),
            BaseElement::new(0x50B2_0004),
        ];
        // El pendiente v2, calculado de forma nativa para comparar.
        let pend_inner = native_merge(receiver_id, salt);
        let c1 = native_merge(
            pend_inner,
            [BaseElement::new(amount), BaseElement::ZERO, BaseElement::ZERO, BaseElement::ZERO],
        );
        let c2 = native_merge(c1, sobre);
        ...
        Scenario {
            public_inputs: ClaimPublicInputs {
                ...
                frozen_root,
                // Antes el pendiente (C2) ESTA en el arbol, despues no.
                pending_root_old: climb_pending(c2),
    ...
    fn trace_de(s: &Scenario) -> TraceTable<BaseElement> {
        build_trace(
            s.key, s.account_id, s.balance, s.nonce, s.leaf_salt, &s.path, &s.frozen_path,
            s.amount, s.supply_old, 0, s.receiver_id, s.salt, s.sobre, &s.pending_path,
        )
    }

    /// Prove + verify contra las entradas publicas de `s`, con el
    /// panico conservado (S25: descartar el mensaje tira el dato).
    fn prueba_verifica(s: &Scenario, trace: TraceTable<BaseElement>) -> bool {
        let prover = ClaimV2Prover::new(default_options());
    ...
    /// La reclamacion v2 legitima verifica.
    #[test]
    fn an_authorized_claim_v2_verifies() {
        let s = scenario(1_000_000, 250_000, 10_000_000);
        assert!(
            prueba_verifica(&s, trace_de(&s)),
            "una reclamacion v2 correcta debe verificar"
        );
    }

    /// Paridad traza<->nativo: C1 en el enlace del cuarto merge, C2 en
    /// la entrada al arbol, el sobre en COL_X donde se lee, y las
    /// entradas publicas DERIVADAS == DECLARADAS (S25: comparar la
    /// estructura entera, no los campos que parecen importantes).
    #[test]
    fn trace_landmarks_match_native() {
        let s = scenario(1_000_000, 250_000, 10_000_000);
        let trace = trace_de(&s);

        let derivadas = ClaimV2Prover::new(default_options()).get_pub_inputs(&trace);
        assert_eq!(
            derivadas.to_elements(),
            s.public_inputs.to_elements(),
            "las entradas publicas DERIVADAS de la traza deben coincidir \
             con las DECLARADAS"
        );
        ...
        for i in 0..4 {
            assert_eq!(
                ...
                c1[i],
                "C1 disponible en el enlace del cuarto merge, elemento {i}"
            );
            assert_eq!(
            ...
            assert_eq!(
                ...
                c2[i],
                "C2 disponible en la entrada al arbol, elemento {i}"
            );
            assert_eq!(
    ...
    fn mutacion_a_un_limbo_del_sobre_alterado_se_rechaza() {
        let s = scenario(1_000_000, 250_000, 10_000_000);
        let mut trace = trace_de(&s);

        let veneno = trace.get(COL_X + 2, ROW_PEND_ENV) + BaseElement::ONE;
        trace.set(COL_X + 2, ROW_PEND_ENV, veneno);

        assert!(
            !prueba_verifica(&s, trace),
            "un limbo del sobre alterado DEBE rechazar (C_ENV_IN); si \
             verifica, el cuarto merge es decorativo"
        );
    }

    /// MUTACION (b): el cuarto merge omitido - C1 desnudo entrando al
    /// camino de pendientes - debe rechazarse. Se escriben AMBAS
    /// mitades del estado siguiente (bit-agnostico); C_PEND_ENTRY_A
    /// dispara sobre la mitad colocada, o la raiz declarada rechaza.
    #[test]
    fn mutacion_b_el_c1_desnudo_no_entra_al_camino() {
        let s = scenario(1_000_000, 250_000, 10_000_000);
        let mut trace = trace_de(&s);

        let c1 = c1_de(&s);
        for i in 0..4 {
            trace.set(4 + i, ROW_PENDING_ENTRY + 1, c1[i]);
            trace.set(8 + i, ROW_PENDING_ENTRY + 1, c1[i]);
        }

        assert!(
            !prueba_verifica(&s, trace),
            "el C1 desnudo NO debe entrar al camino de pendientes; si \
             verifica, el sobre no envuelve nada"
        );
    }

    /// NO SE PUEDE COBRAR UN IMPORTE DISTINTO DEL COMPROMETIDO -
    /// tambien bajo v2: cambiar el importe da otro C1, otro C2, y la
    /// subida no llega a la raiz declarada.
    #[test]
    fn claiming_a_different_amount_is_rejected() {
        let s = scenario(1_000_000, 250_000, 10_000_000);
        let trace = build_trace(
            s.key, s.account_id, s.balance, s.nonce, s.leaf_salt, &s.path, &s.frozen_path,
            s.amount * 2, s.supply_old, 0, s.receiver_id, s.salt, s.sobre, &s.pending_path,
        );
        assert!(
            !prueba_verifica(&s, trace),
    ...
    /// DOMINIO (RFC-0003): un arbol con el pendiente v1 (C1, sin
    /// sobre) NO se cobra con el circuito v2 - el legado es inmune por
    /// dominio, no por marca. El v2 reconstruye C2 y la subida no
    /// llega a la raiz del arbol v1.
    #[test]
    fn un_arbol_con_c1_no_se_cobra_con_el_v2() {
        let mut s = scenario(1_000_000, 250_000, 10_000_000);
        let c1 = c1_de(&s);
        s.public_inputs.pending_root_old = climb_pending_de(&s, c1);

        assert!(
            !prueba_verifica(&s, trace_de(&s)),
            "CRITICO: un pendiente v1 (C1) no debe poder cobrarse con la \
