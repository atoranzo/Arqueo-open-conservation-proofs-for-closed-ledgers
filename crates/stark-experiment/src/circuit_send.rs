...
//! 3. **No está congelado**: no-pertenencia al árbol de congelados. ⚠️ §487:
//!    lo que el AIR prueba es que ALGUNA posición de ese árbol tiene hoja
//!    cero, no la del titular (`COL_FBIT` no está atada a `COL_BIT`). La
//!    no-congelación la impone la capa al aplicar (S487); el arreglo B la
//!    devolverá al AIR.
...
pub const TRACE_WIDTH: usize = 58;
...
/// **La envoltura de la hoja (§117, piloto B13/B14).** Seis familias
/// cosidas por `link_salt` en `ROW_SALT_LINK`: capacidad a cero, digest
...
const C_IACC_STEP: usize = C_LIMIT_CONST + 1; // 1
...
mod tests {
    ...
    /// del circuito en su escenario honesto — construcción + prove dentro
    ...
    fn medicion_130_send() {
        ...
        println!(
            "[§130] send gemelo: prove {ms:.1} ms, proof {} bytes",
            proof.to_bytes().len()
        );
    }
