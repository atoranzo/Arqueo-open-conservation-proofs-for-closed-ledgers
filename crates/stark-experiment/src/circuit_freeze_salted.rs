//! ⚠️ **ANDAMIO B13/B14 — GEMELO del mundo nuevo (paso 3, §151).**
//!
//! Copia declarada de `circuit_freeze` — SIN salt (el árbol de
//! congelados no tiene hojas de cuenta): aquí el mundo nuevo es la
//! PROFUNDIDAD. Este módulo es la CASA de `FROZEN_DEPTH`: el gemelo
//! define **32** (§128) y toda la geometría se re-deriva de la
//! constante (playbook R2, luego R6 = una línea + mutaciones de
//! profundidad).
//!
//! **Cláusula de retirada**: en el flip (release única, D4) este módulo
//! SUSTITUYE a `circuit_freeze` y el legacy se borra. Hasta entonces,
