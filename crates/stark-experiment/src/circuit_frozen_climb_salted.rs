//! ⚠️ **ANDAMIO B13/B14 — GEMELO del mundo nuevo (paso 3, §152).**
//!
//! Copia declarada de `circuit_frozen_climb` — SIN salt: la subida dual
//! del árbol de congelados, a PROFUNDIDAD 32 (§128) con constante
//! LOCAL (auto-contenido: los gemelos no importan de otros gemelos; en
//! el flip, `circuit_freeze` exporta 32 y el import vuelve). A 32 el
//! ajuste es EXACTO: `ROW_ROOT = 255 = TRACE_LENGTH − 1` — el relleno
//! caro de §60.2 se desvanece.
//!
//! **Cláusula de retirada**: en el flip (release única, D4) este módulo
//! SUSTITUYE a `circuit_frozen_climb` y el legacy se borra. Hasta
...
mod tests {
    ...
    fn medicion_130_frozen_climb_gemelo() {
        ...
        println!(
            "[§130] frozen_climb gemelo: prove {ms:.1} ms, proof {} bytes",
            proof.to_bytes().len()
        );
    }
}
