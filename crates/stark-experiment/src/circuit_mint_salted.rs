//! ⚠️ **ANDAMIO B13/B14 — GEMELO del mundo nuevo (paso 3, §146).**
//!
//! Copia declarada de `circuit_mint` donde vive su migración — hoja
//! envuelta y las mutaciones obligatorias — por el playbook
//! (`doc/playbook-replica-gemelos.md`). Sin fase frozen: **R6 no
//! aplica** (checklist de la spec: mint | salt sí | frozen-32 —). La
//! fase de custodios 2-de-N queda INTACTA: el salt vive en la máquina
//! de hoja, ciclos 0..2, y el calendario posterior solo se corre.
//!
//! **Cláusula de retirada**: en el flip (release única, D4) este módulo
//! SUSTITUYE a `circuit_mint` y el legacy se borra. Hasta entonces,
