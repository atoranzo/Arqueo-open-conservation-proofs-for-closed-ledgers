//! ⚠️ **ANDAMIO B13/B14 — el GEMELO del mundo nuevo (paso 2, §142).**
//!
//! Copia declarada de `circuit_send` donde vive el piloto — hoja
//! envuelta (`native_leaf_salted`), frozen-32 local y las mutaciones
//! obligatorias — sin tocar el circuito vivo: la capa sigue importando
//! `circuit_send` y su suite sigue informando durante toda la campaña
//! (estrategia C; precedente: los shims con retirada de 49-A).
//!
//! **Cláusula de retirada**: en el flip (release única, spec de
//! migración D4) este módulo SUSTITUYE a `circuit_send` y el legacy se
...
mod tests {
    ...
    /// del GEMELO en su escenario honesto — construcción + prove dentro
