    ...
    VC: VectorCommitment<HashFn>,
{
    ...
    if proof.trace_info().meta() == ::air::MARCA_OCULTA {
        ...
        let air = ::air::Oculta::<AIR>::new(
            proof.trace_info().clone(),
            pub_inputs,
            proof.options().clone(),
        );
        return match air.options().field_extension() {
            FieldExtension::None => {
                let public_coin = RandCoin::new(&public_coin_seed);
                let channel = VerifierChannel::new(&air, proof)?;
                perform_verification::<::air::Oculta<AIR>, AIR::BaseField, HashFn, RandCoin, VC>(
                    air,
                    channel,
                    public_coin,
                )
    ...
    V: VectorCommitment<H>,
{
    ...
    let ood_constraint_evaluation_2 = ood_constraint_evaluations
        ...
        .fold(E::ZERO, |result, (i, &value)| {
            result + z.exp_vartime(((i * paso) as u32).into()) * value
        });
