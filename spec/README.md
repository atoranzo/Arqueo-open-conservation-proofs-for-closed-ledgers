...
| `rfc/0008-pruebas-portables-del-pendiente.md` | RFC-0008, PROPOSED: the two portable proofs of a pending item — the payee proves, under a signed head v5 with the path inside the circuit, that a pending item exists in their name for at least X (E1); the payer proves, from the same head, an exact amount to a named recipient and no reversal before T (E2); the pledge as a transition with proof, a label of its own domain in the consumption tree, so a second pledge is `ConsumoRepetido` (E3); catalogue and bench (E4). Wire stays `zkssl/0.3`; E1 sealed (§484–§495) and the payee's side of E4 sealed (§496–§499: the CLI collector's mouth, the bench and `vectors/pendiente/`); the payer's side comes with E2 | you read what payee and payer will each prove about one in-flight payment, and what a pledge does and does not oblige |
