...
- **Estado:** ACEPTADO — **la regla 4 del PROCESO, saldada con medida** (§483 la adopción; E1
  §484–§495, E2 §501–§509 y E4 §496–§499 con §508 y §509, las etapas con sus puertas; E3
  §513–§520 el diseño y el código, §541–§545 la boca, el SDK y el banco, y §546 el catálogo, la
  octava familia del artefacto y del canon; §547 este giro). La spec está al día (`PAQUETE.md`
  2.8, 2.9, 2.10 y la sección 9), los vectores son nuevos bajo `zkssl/0.4` —`pendiente/`, `pago/`
  y `prenda/`, nueve cada uno— y las suites están verdes: el canon los corre por el arnés, y
  también desde dentro del tarball. El OpenRPC no se regenera porque ningún método se ha movido
...
- **Asiento(s) de AUDITORIA:** §121, §178, §211, §275 (el acuse y su raíz), §343–§345 (el
  ...
  de E4 por los dos lados y anota la cota. Y el arco de E3: el §513, que separa la marca de la
  prenda; el §514 y el §515, que deciden D-AV..D-AY; el §516 y el §517, el AIR y el probador; el
  §518, el productor; el §519, el cable `zkssl_pledge`; el §520, el octavo brazo del mando; el
  §521 y el §538, que la paran y la desbloquean por el 5.A-360; el §541, que escribe aquí
  D-BC..D-BI; el §542 y el §543, el SDK y la boca; el §544, el quinto fichero de la siembra; el
  §545, el banco; el §546, el catálogo; y el §547, este giro.
...
| E3 — la prenda, como transición con prueba | el receptor marca el pendiente como prendado: una MARCA con dominio propio sobre `C2` en el árbol de consumos, publicada por un método aditivo, `zkssl_pledge`, que EXIGE el sobre de PRENDA: AIR propio (D-AV) que publica la MARCA con `C2` de testigo (D-AW), sin la banda y sin la meta, en 42 columnas y 512 filas (D-AX, D-AY); una segunda MARCA es `ConsumoRepetido`, que ya tiene sobre de rechazo con prueba (RFC-0007 E3, `PAQUETE.md` 2.6). La PRENDA es el par marca + sobre (D-AS), y no toca el cobro ni el reembolso: lo que obliga es contrato, y se declara | NO | **sellada** — §513 (la marca y la prenda son dos objetos), §514 (D-AV, el AIR propio) y §515 (D-AW, D-AX y D-AY: la marca publicada, la clave donde estaba la banda, sin meta), §516 y §516-B (el AIR con su juez), §517 y §517-B (el probador y su primera prueba real), §518 y §518-B (el productor en la capa), §519 y §519-B (el cable `zkssl_pledge`, que verifica antes de escribir) y §520 y §520-B (el octavo brazo del mando); PARADA entre el §521 y el §538 por el 5.A-360 —la prueba publicaba la clave de gasto del prendador 42 veces— y reabierta cuando el RFC-0009 E3b-2 la ocultó; §541 (D-BC..D-BI, la boca), §542 (`Wallet::prueba_de_prenda`), §543 (la boca `zk-ssl-cli prueba-prenda`), §544 (el keystore del receptor, quinto fichero de la siembra), §545 (el banco `tools/banco_prenda.sh`) y §546 (el catálogo `spec/vectors/prenda/`, octava familia). **E3 queda entera**, y es MEDIA prenda dicha en voz alta: el par con la marca bajo el `consRoot` lo cierra `zkssl_consumoPath` (D-AS) |
| E4 — el catálogo y el banco, por lados | `spec/vectors/pendiente/`: dos positivos por lado, REUNIDOS de las capturas de un nodo real (molde: `edad/`), y un negativo por regla producible; `MANIFIESTO.txt`; la familia en `FAMILIAS`; el banco que lo reproduce en vivo; la sección 9 de `PAQUETE.md`. Va POR LADOS en una sola fila (D-O): el del cobro primero —sus dos formas (D-N), la boca del cli (D-P) y sus negativos (D-Q)— y el del pago con E2; esta celda nombra lo sellado de cada lado. El giro a ACEPTADO exige la regla 4 medida letra a letra, como el §481 | NO | **sellada por el lado del cobro** — §496 (D-N..D-Q), §497 y §497-B (la boca del cobrador en el cli, `prueba-cobro`, y `simulate --v2`), §498 (D-R..D-W y el banco, `tools/banco_pendiente.sh`) y §499 (D-X..D-AC y el catálogo, `spec/vectors/pendiente/`, sexta familia del artefacto y del canon). **Y el lado del PAGO**: §508 (el banco propio, `tools/banco_pago.sh`, con su siembra de cuatro ficheros, D-AP y D-AQ) y §509 (el catálogo `spec/vectors/pago/`, séptima familia del artefacto y del canon, D-AR, que revierte D-AC y la letra de D-AJ). **Los dos lados están; la etapa queda entera**, y la regla 4, medida letra a letra, la saldó el §546 con la octava familia: el §547 gira el documento |
